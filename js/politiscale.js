...
function saveCache() {
  try { localStorage.setItem(CACHE_KEY, JSON.stringify({ answers, currentQ })); } catch(e) {}
}
function loadCache() {
  try {
...
function drawCompass(n) {
  ...
  svg.innerHTML = `
    <defs>
      ...
      <clipPath id="circle-clip2"><circle cx="${cx}" cy="${cy}" r="${r}"/></clipPath>
    </defs>
  ...
  AXES.forEach(ax=>{
    ...
    row.innerHTML=`
      <div class="axe-header">
        ...
        <span style="color:${color};font-size:.8rem;font-family:var(--f-mono)">${label} · ${Math.round(Math.abs(val))} %</span>
        <span>${ax.right}</span>
      </div>
...
function buildPartis(n) {
  ...
  scored.forEach(p=>{
    ...
    card.innerHTML=`
      ...
      <div class="compat-bar-wrap"><div class="compat-bar" style="width:0%;background:${p.color}" id="bar-parti-${p.name.replace(/\s/g,'_')}"></div></div>
      <div class="compat-pct">${p.compat} % de compatibilité</div>
    `;
    container.appendChild(card);
    setTimeout(()=>{
      const el=document.getElementById(`bar-parti-${p.name.replace(/\s/g,'_')}`);
      if(el) el.style.width=p.compat+'%';
    },150);
  });
}
...
function buildPenseurs(n) {
  ...
  scored.forEach(p=>{
    ...
    card.innerHTML=`
      ...
      <div class="compat-pct">${p.compat} % d'affinité</div>
    `;
    container.appendChild(card);
  });
}
...
function downloadResult() {
  ...
  // Boussole
  _drawCompassCanvas(ctx, n, 730, 180, 100);
  ...
  AXES.forEach((ax, i) => {
    ...
    ctx.fillStyle = color;
    ctx.textAlign = 'right';
    ctx.fillText(Math.round(intensity) + '%', x + colW, y);
    ctx.textAlign = 'left';
  ...
  ctx.textAlign = 'left';
  ctx.fillText('politilab · ' + new Date().toLocaleDateString('fr-FR'), 32, H - 16);
...
function _drawCompassCanvas(ctx, n, cx, cy, r) {
  ...
  grad.addColorStop(0, '#1a1a2e'); grad.addColorStop(1, '#0d0d18');
  ...
  ctx.font='600 8px monospace'; ctx.fillStyle='rgba(255,255,255,0.38)';
  ctx.textAlign='center';
  ctx.fillText('AUTORITAIRE',cx,cy-r-5); ctx.fillText('LIBERTAIRE',cx,cy+r+12);
  ctx.textAlign='right'; ctx.fillText('COLL.',cx-r-3,cy+3);
  ctx.textAlign='left'; ctx.fillText('LIB.',cx+r+3,cy+3); ctx.textAlign='left';
  const px=cx+(n.eco/100)*r, py=cy-(n.auth/100)*r;
...
function _wrapText(ctx,text,x,y,maxW,lh) {
  const words=text.split(' '); let line='',ly=y,lc=0;
  for(let i=0;i<words.length;i++){
    const test=line+words[i]+' ';
    if(ctx.measureText(test).width>maxW&&i>0){
      ctx.fillText(line.trim(),x,ly); line=words[i]+' '; ly+=lh; lc++;
      if(lc>=3){ctx.fillText(line.trim()+'…',x,ly);return;}
    } else {line=test;}
  }
...
(function init() {
  ...
  banner.innerHTML = `
    ...
    <button class="btn btn-primary btn-sm" id="banner-resume">${isComplete ? 'Voir mes résultats' : 'Reprendre'}</button>
  ...
  document.getElementById('banner-clear').onclick = () => {
    clearCache(); answers.fill(null); currentQ=0; banner.remove();
  };
})();
